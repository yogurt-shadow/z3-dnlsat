...
namespace nlsat {
    ...
    struct solver::imp {
        ...
        imp(solver& s, ctx& c):
            ...
            m_dm(m_am, m_pm, m_assignment, s, m_clauses, m_learned, m_atoms, m_restarts, m_learned_deleted),
        ...
        */
        ...
        void undo_new_stage() {
            var curr = m_dm.pop_last_var();
            if(m_dm.assigned_size() == 0){
                m_xk = null_var;
            }
            else{
                m_xk = m_dm.get_last_assigned_var();
                m_assignment.reset(m_xk);
                m_dm.undo_watched_clauses(m_xk);
            }
        }
        ...
        void undo_until_stage(var new_xk) {
            // wzh dynamic
            var stage_var = (new_xk == null_var) ? null_var : m_dm.get_assigned_var(new_xk);
        ...
        */
        ...
        void select_inc_var(){
            ...
            if(m_xk == null_var){
                ...
                if(m_dm.assigned_size() >= num_vars()){
                    m_xk = null_var;
                }
                else{
                    m_xk++;
        ...
        */
        ...
        bool is_satisfied() {
            ...
            if(m_bk == null_bool_var && m_dm.assigned_size() > num_vars()){
        ...
        */
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (conflict_clause == nullptr){
                        ...
                        //     TRACE("wzh", tout << "[reduce] reduceDB" << std::endl;);
                        //     reduceDB();
                        //     TRACE("wzh", tout << "[reduce] exit reduceDB" << std::endl;);
                        // }
                        break;
                    }
        ...
        void process_antecedent(literal antecedent) {
            ...
            if (!is_marked(b)) {
                ...
                if(b_lvl == scope_lvl() && m_dm.same_stage_bool(b, m_xk)){
        ...
        */
        bool only_literals_from_previous_stages(unsigned num, literal const * ls) {
            for (unsigned i = 0; i < num; i++) {
                ...
                //     return false;
                if(m_dm.same_stage_literal(ls[i], m_xk)){
                    return false;
                }
            }
            return true;
        ...
        */
        void remove_literals_from_lvl(scoped_literal_vector & lemma, unsigned lvl) {
            ...
            for (unsigned i = 0; i < sz; i++) {
                ...
                if (assigned_value(l) == l_false && m_levels[b] == lvl && m_dm.same_stage_bool(b, m_xk)) {
                    // TRACE("wzh", tout << "[debug] loop literal: ";
                    //     display(tout, l);
                    //     tout << std::endl;
                    // );
        ...
        */
        unsigned find_new_level_arith_lemma(unsigned sz, literal const * lemma) {
            ...
            for (unsigned i = 0; i < sz - 1; i++) {
                literal l = lemma[i];
                // if (max_var(l) == m_xk) {
                if(m_dm.same_stage_literal(l, m_xk)){
                    bool_var b = l.var();
                    if (!found_lvl) {
                        found_lvl = true;
                        new_lvl   = m_levels[b];
